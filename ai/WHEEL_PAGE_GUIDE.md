...
- **H1 standard (2026-08-05, all wheel pages)**: every `<h1>` across all 6 wheel pages (both families) now uses `text-[16px] md:text-[20px] lg:text-[24px] font-sans tracking-[6px] md:tracking-[9px] lg:tracking-[12.8px]` — small, letter-spaced, plain system sans-serif, matching `/process/forged`'s heading treatment and no longer the large `font-franklin-heavy` display treatment the templates originally used. This was rolled out from `laguna.astro` to every other page by explicit user request — use this exact class string for any new wheel page's H1 too, don't revert to `font-franklin-heavy`.
